...
const toBeClonable = exports.toBeClonable = function toBeClonable(received, expected) {
  // Check if the matcher is being used with .not and throw an error since it's not supported
  if (this.isNot) {
    throw new Error(_messages.messages.negationError('toBeClonable'));
  }
  ...
  if (expected !== null && expected !== undefined && typeof expected !== 'object') {
    return {
      pass: false,
      message: () => _messages.messages.invalidObjectParam('options')
    };
  }
  ...
  const isInstanceOfGesture = clone instanceof _core.Gesture;
  ...
  const pass = isNotSameInstance && isInstanceOfGesture && overridesApplied && nonOverriddenPropertiesMatch;
  if (pass) {
    return {
      pass: true,
